...
namespace YieldStudio\FilamentPanel\Tables\Columns;
...
{
    protected string $view = 'yield-panel::columns.progress-bar-column';
    ...
    protected function setUp(): void
    {
        ...
        $this->dangerLabel = fn ($state): string => $state <= 0 ? 'out of stock' : $state;
        $this->warningLabel = fn ($state): string => $state !== null ? $state . ' low stock' : '0 low stock';
        $this->successLabel = fn ($state): string => $state !== null ? $state . ' in stock' : '0 in stock';
    }
